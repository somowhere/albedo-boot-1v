    ...
    <div class="avue-data-display">
      ...
      <el-row :span="24" :gutter="30">
        ...
        <el-col :span="8">
          ...
          <div class="row">
            ...
            <div class="col-md-3 text-right"><el-tag>{{getVal(metrics.gauges['jvm.garbage.PS-MarkSweep.count']) }}</el-tag></div>
          </div>
          <div class="row">
            <div class="col-md-9">标记清除耗时</div>
            <div class="col-md-3 text-right"><el-tag>{{getVal(metrics.gauges['jvm.garbage.PS-MarkSweep.time']) }}ms</el-tag></div>
          </div>
          <div class="row">
            <div class="col-md-9">回收次数</div>
            <div class="col-md-3 text-right"><el-tag>{{getVal(metrics.gauges['jvm.garbage.PS-Scavenge.count']) }}</el-tag></div>
          </div>
          <div class="row">
            <div class="col-md-9">回收耗时</div>
            <div class="col-md-3 text-right"><el-tag>{{getVal(metrics.gauges['jvm.garbage.PS-Scavenge.time']) }}ms</el-tag></div>
          </div>
        </el-col>
      </el-row>
      <h3>HTTP 请求 (事件 / 秒)</h3>
      ...
      <div class="table-responsive" v-show="!updatingMetrics">
        <table class="el-table">
          ...
          <tbody>
          <tr v-for="(entry, key,index) in servicesStats">
            <td>{{key}}</td>
      ...
      <el-dialog :title="'线程转储'" :visible.sync="dialogVisible">
        <el-button-group>
        <el-button @click="threadDumpFilterByState('ALL')" type="primary" size="mini">All&nbsp;<span class="badge badge-pill badge-default">{{threadDumpAll}}</span></el-button>
        <el-button @click="threadDumpFilterByState('RUNNABLE')" type="success"size="mini">Runnable&nbsp;<span class="badge badge-pill badge-default">{{threadDumpRunnable}}</span></el-button>
        <el-button @click="threadDumpFilterByState('WAITING')" type="info" size="mini">Waiting&nbsp;<span class="badge badge-pill badge-default">{{threadDumpWaiting}}</span></el-button>
        <el-button @click="threadDumpFilterByState('TIMED_WAITING')" type="warning"size="mini">Timed Waiting&nbsp;<span class="badge badge-pill badge-default">{{threadDumpTimedWaiting}}</span></el-button>
        <el-button @click="threadDumpFilterByState('BLOCKED')" type="danger"size="mini">Blocked&nbsp;<span class="badge badge-pill badge-default">{{threadDumpBlocked}}</span></el-button>
        </el-button-group>
        <el-input v-model="threadDumpFilter" style="margin: 10px 0 10px 0;"></el-input>
        <div style="margin-bottom: 10px;" v-for="(entry,index) in threadDump" >
          <h6>
          ...
          <div class="card" v-if="entry.show">
            <div class="card-body">
              <div v-for="(st,index) in entry.stackTrace" class="break">
